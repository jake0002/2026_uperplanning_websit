...
const cleanIconsBlock = `  <!-- 5. DESKTOP WORKSPACE & HORIZONTAL ICON BAR -->
  <div id="desktop">
    <div id="iconsArea">
      <!-- 1. 슈퍼플래닝 소개 -->
...
const sectionRegex = /<!-- 5\. DESKTOP WORKSPACE & HORIZONTAL ICON BAR -->[\s\S]*?(?=<!-- 6\. WINDOW AREA)/;
if (sectionRegex.test(content)) {
  content = content.replace(sectionRegex, cleanIconsBlock + '\n\n  ');
  fs.writeFileSync(indexPath, content, 'utf8');
  console.log('Cleanly replaced desktop icons section in index.html');
} else {
  console.error('Could not match desktop section regex');
}
