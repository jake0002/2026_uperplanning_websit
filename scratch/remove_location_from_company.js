...
companyFiles.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        // 1. Remove item 5 from Tree Sidebar TOC and renumber item 6 to 5
        ...
        const newTreeSidebar = `<aside class="tree-sidebar" aria-label="탐색기 목차 트리">
          ...
          <ul class="tree-list toc">
            ...
            <li class="tree-item"><a href="#faq" class="tree-link"><span class="doc-icon">📄</span> <span>5. 자주 묻는 질문 (FAQ)</span></a></li>
          </ul>
        </aside>`;

        content = content.replace(oldTreeSidebar, newTreeSidebar);

        // 2. Remove item 5 from Main Content Pane and renumber item 6 to 5
        ...
        // 3. Update statusbar count from 6개 개체 to 5개 개체
        content = content.replace('<div>6개 개체</div>', '<div>5개 개체</div>');

        if (content !== original) {
            fs.writeFileSync(filepath, content, 'utf8');
            console.log(`Removed location from: ${filepath}`);
        }
    }
});
