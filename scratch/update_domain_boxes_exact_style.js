...
const exactBoxesHTML = `
          <div class="grid-boxes two" style="margin-top: 20px;">
            <div style="border: 1px solid #ccc; padding: 20px; background-color: #fff;">
              <h5 style="margin: 0 0 16px 0; font-size: 16px; color: #000; font-weight: bold;">금융, 공공 서비스</h5>
            ...
            <div style="border: 1px solid #ccc; padding: 20px; background-color: #fff;">
              <h5 style="margin: 0 0 16px 0; font-size: 16px; color: #000; font-weight: bold;">플랫폼, 모빌리티, 기업 서비스</h5>
...
files.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        // Replace old grid-boxes block in Section 6
        content = content.replace(/<div class="grid-boxes two" style="margin-top: 20px;">[\s\S]*?<\/article>\s*<\/div>/g, exactBoxesHTML.trim());
        content = content.replace(/<div class="grid-boxes two" style="margin-top: 20px;">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g, exactBoxesHTML.trim());

        if (content !== original) {
            fs.writeFileSync(filepath, content, 'utf8');
            console.log(`Updated Section 6 domain boxes to exact 제공산출물 style in: ${filepath}`);
        } else {
