...
files.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        let targetP = '설계안을 단순 결과물이 아니라 조직 내 커뮤니케이션 도구로도 쓸 수 있게 만든다는 것에 많은 고객사 분들의 만족도를 높여주었습니다.</p>';
        let pIdx = content.indexOf(targetP);
        if (pIdx !== -1) {
            let insertPos = pIdx + targetP.length;
            ...
            console.log(`Could not find target paragraph in: ${filepath}`);
        }
    }
});
