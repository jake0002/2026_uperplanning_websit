...
const restoredContentCleanroomHTML = `\n
<p style="margin-top: 20px;">${p1}</p>
<p>${p2}</p>
</section>
...
files.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        if (content.includes('<h2 id="faq">')) {
            console.log(`FAQ section already present in: ${filepath}`);
            return;
        }

        let boxSearchStr = '플랫폼, 모빌리티, 기업 서비스 사례';
        let boxIdx = content.indexOf(boxSearchStr);
        if (boxIdx !== -1) {
            let divEnd = content.indexOf('</div>', boxIdx);
            if (divEnd !== -1) {
                let outerDivEnd = content.indexOf('</div>', divEnd + 6);
                if (outerDivEnd !== -1) {
                    let insertPos = outerDivEnd + 6;
                    let toInsert = filepath.includes('cleanroom') ? restoredContentCleanroomHTML : restoredContentHTML;
                    content = content.substring(0, insertPos) + toInsert + content.substring(insertPos);
                    fs.writeFileSync(filepath, content, 'utf8');
                    console.log(`Restored paragraphs and FAQ section in: ${filepath}`);
                }
            }
        } else {
            console.log(`Could not find domain boxes in: ${filepath}`);
        }
    }
});
