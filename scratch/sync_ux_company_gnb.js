...
// 2. Update .gnb-link:hover and active CSS
content = content.replace(
  /\.gnb-link:hover,\s*\.gnb-link\.active\s*\{[\s\S]*?\}/,
  `.gnb-link:hover,
    .gnb-link:active,
    .gnb-link.active {
      background: #000000 !important;
...
// 3. Update Progress Track & Bar & Badge CSS
content = content.replace(
  /\.gnb-progress-track\s*\{[\s\S]*?pointer-events: none;\s*\}/,
  `.gnb-progress-track {
      position: absolute;
      bottom: 0;
      left: 0;
...
content = content.replace(
  /\.gnb-progress-bar\s*\{[\s\S]*?transition: width 0\.08s cubic-bezier\(0\.1, 0\.7, 0\.1, 1\);\s*\}/,
  `.gnb-progress-bar {
      height: 100%;
      width: 0%;
      background: #000000;
...
content = content.replace(
  /\.gnb-progress-bar::after\s*\{[\s\S]*?opacity: 0\.95;\s*\}/,
...
content = content.replace(
  /\.gnb-progress-badge\s*\{[\s\S]*?pointer-events: none;\s*\}/,
  `.gnb-progress-badge {
      position: absolute;
      right: -14px;
      top: 10px;
