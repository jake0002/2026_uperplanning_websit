...
// Insert bannerHtml inside <div class="page-wrapper"> right after </div> of .explorer-window
const statusbarMarker = `<div class="statusbar">`;
const statusbarIdx = html.indexOf(statusbarMarker);

if (statusbarIdx !== -1) {
  // Find closing </div> of .explorer-window which comes after statusbarIdx
  const explorerCloseIdx = html.indexOf('</div>', statusbarIdx + 100);
  if (explorerCloseIdx !== -1) {
    html = html.substring(0, explorerCloseIdx + 6) + '\n' + bannerHtml + html.substring(explorerCloseIdx + 6);
  } else {
    console.log('Error finding explorerCloseIdx');
  }
} else {
  console.log('Error finding statusbarMarker');
}
