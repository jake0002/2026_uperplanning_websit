...
files.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        let captionIdx = content.indexOf('📷 특허 다수 보유 UX전문 에이전시 슈퍼플래닝');
        if (captionIdx !== -1) {
            let divEndIdx = content.indexOf('</div>', captionIdx);
            if (divEndIdx !== -1) {
                let insertPos = divEndIdx + 6;
                let textToInsert = filepath.includes('cleanroom') ? paragraphCleanroomHTML : paragraphHTML;
                content = content.substring(0, insertPos) + textToInsert + content.substring(insertPos);
            }
        }

        if (content !== original) {
            fs.writeFileSync(filepath, content, 'utf8');
            console.log(`Inserted 3 paragraphs in: ${filepath}`);
        } else {
