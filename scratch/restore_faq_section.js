...
files.forEach(filepath => {
    if (fs.existsSync(filepath)) {
        ...
        if (content.includes('id="faq"') || content.includes('7. 자주 묻는 질문')) {
            console.log(`FAQ section already exists in: ${filepath}`);
            return;
        }
        ...
        if (filepath.includes('cleanroom')) {
            ...
            if (mainEnd !== -1) {
                content = content.substring(0, mainEnd) + faqCleanroomHTML + '\n' + content.substring(mainEnd);
            }
        } else {
            let mainEnd = content.indexOf('</main>');
            if (mainEnd !== -1) {
                content = content.substring(0, mainEnd) + faqHTML + '\n\n        ' + content.substring(mainEnd);
            }
        }
